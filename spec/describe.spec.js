...
const outerBeforeEachFn = jest.fn(() => fakeContext())
const innerBeforeAllFn = jest.fn(() => fakeContext())
const innerBeforeEachFn = jest.fn(() => fakeContext())
...
test('does not leak group context', (context) => {
  ...
  const innerGroupContext = innerBeforeAllFn.mock.results[0].value
  expect(context).not.toMatchObject(innerGroupContext)
...
test('does not leak test context', (context) => {
  ...
  expect(context).not.toMatchObject(innerTestContext)
})
